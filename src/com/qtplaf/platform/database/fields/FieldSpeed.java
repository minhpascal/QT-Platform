...
/**
 * Speed field on an instrument. Generated by a speed configuration.
 *
 * @author Miquel Sas
 */
public class FieldSpeed extends Field {

	/**
	 * Constructor.
	 */
	public FieldSpeed(Session session, Speed speed, String name) {
		super(Domains.getDouble(session, name));
		String suffix = StringUtils.getSuffix(name, "_");
		setHeader(speed.getHeader() + " " + StringUtils.capitalize(suffix));
		setLabel(speed.getLabel() + " " + StringUtils.capitalize(suffix));
		setFormatter(Formatters.getValueFormatter(session, suffix));
		setProperty(Fields.Properties.Speed, speed);
	}
}
